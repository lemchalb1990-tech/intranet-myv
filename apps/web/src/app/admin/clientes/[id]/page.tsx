...
function DeleteClientModal({
  ...
  const [loading, setLoading] = useState(false);
  const { formatRut, normalizeRut } = require("@/lib/rut");
  const expected = normalizeRut(rut);
  const match = normalizeRut(input.replace(/[^0-9kK]/g, "")) === expected;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-xl border border-slate-200 shadow-lg w-full max-w-sm">
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100">
          <h2 className="font-semibold text-slate-800">Eliminar cliente</h2>
          <button onClick={onClose} className="p-1 rounded-lg hover:bg-slate-100 text-slate-400">
            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
          </button>
        </div>
        <div className="px-5 py-4 space-y-4">
          <div className="bg-red-50 border border-red-100 rounded-lg px-3 py-2.5">
            <p className="text-sm text-red-700 font-medium">Esta acción es irreversible.</p>
            <p className="text-xs text-red-500 mt-0.5">Se eliminarán todos los proyectos, documentos y datos del cliente.</p>
          </div>
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1">
              Ingresa el RUT del cliente para confirmar
            </label>
            <input
              type="text"
              placeholder={formatRut(rut)}
              value={input}
          ...
          <div className="flex gap-2 pt-1">
            ...
            <button
              ...
              className="flex-1 py-2 bg-red-600 text-white rounded-lg text-sm hover:bg-red-700 disabled:opacity-40 transition"
            >
